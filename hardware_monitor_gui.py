...
from textual.widgets import Header, Footer, Static, Button, TabbedContent, TabPane, Tabs, Tab
        ...
        while True:
            if self.race.state == RaceState.RUNNING:
                if self._race_start_time is None:
                    self._race_start_time = asyncio.get_event_loop().time()
                    # Fix lap times once at start for each racer
                    for racer in self._racer_ids:
                        self._racer_lap_times[racer] = 5.0 + random.uniform(-1, 1)
                    print(f"Race started at {self._race_start_time}")
                    print(f"Lap times per racer: {self._racer_lap_times}")

                now = asyncio.get_event_loop().time()
                elapsed = now - self._race_start_time
                ...
                for racer_id, lap_number, lap_time in next_lap_events:
                    print(f"Emitting lap {lap_number} for racer {racer_id} at lap time {lap_time}")
                ...
                if self._race_start_time is not None:
                    print("Race stopped/reset")
    ...
    async def refresh_lap_data(self):
        ...
        while True:
            try:
                ...
                lap_display_events.refresh()
                lap_display_leaderboard.leaderboard = self.race.leaderboard()
                lap_display_leaderboard.refresh()
                ...
                lap_display_leaderboard.leaderboard = self.race.leaderboard()
                lap_display_leaderboard.refresh()
            if self.race.state == RaceState.RUNNING and self._race_start_time is not None:
                race_time_display.elapsed_time = asyncio.get_event_loop().time() - self._race_start_time
            else:
                race_time_display.elapsed_time = 0.0
            race_time_display.refresh()
            await asyncio.sleep(0.1)
        ...
        with Container(id="main_container"):
            with Vertical():
                with Horizontal(id="race_controls"):
                    ...
                    yield RaceTimeDisplay(id="race_time")
                    yield RaceStatusDisplay(id="status_bar")
